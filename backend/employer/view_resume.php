<?php
// backend/employer/view_resume.php
// API to view applicant resumes in browser
...
try {
    ...
    logActivity("RESUME_VIEW", "Viewed resume for application {$application_id}");
    
    // Determine content type and viewing method
    $file_extension = strtolower(pathinfo($resume['file_name'], PATHINFO_EXTENSION));
    
    if ($file_extension === 'pdf') {
        // For PDF files, display directly in browser
        header('Content-Type: application/pdf');
        header('Content-Disposition: inline; filename="' . basename($resume['file_name']) . '"');
    } else if (in_array($file_extension, ['doc', 'docx'])) {
        // For Word documents, force download as browsers can't display them directly
        header('Content-Type: application/vnd.openxmlformats-officedocument.wordprocessingml.document');
        header('Content-Disposition: attachment; filename="' . basename($resume['file_name']) . '"');
    } else {
        // For other file types, try to display as is
    ...
    die('Error viewing resume: ' . $e->getMessage());
}